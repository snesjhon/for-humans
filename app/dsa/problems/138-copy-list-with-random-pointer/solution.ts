...
function copyRandomList(head: RandomListNode | null): RandomListNode | null {
  ...
  // Pass 1: build the copied street and log every old house's new address.
  ...
  currentOld = head; // restart the original walker at the front of the old street
  let currentCopy = dummy.next; // start the copied walker at the front of the new street

  // Pass 2: translate each private note through the forwarding ledger.
...
runMutationCase('mutating the original after cloning does not mutate the copy', [[8, 1], [9, 0]]);
...
function runMutationCase(desc: string, input: RandomPair[]): void {
  const original = buildList(input);
  const clone = copyRandomList(original);
