...
export default function SettingsPage() {
  ...
  return (
    <main className="max-w-[600px] mx-auto mt-[80px] px-6">
      ...
      <p className="text-[var(--ms-text-subtle)] mb-[2rem] text-[0.9375rem]">
        Your API key is stored in your browser only. It never leaves your device.
      </p>

      {stored && (
        <div className="mb-[1.5rem] py-[12px] px-[16px] rounded-[6px] bg-[var(--ms-bg-pane-secondary)] border border-[var(--ms-surface)] flex justify-between items-center">
          <span className="font-[ui-monospace,monospace] text-[0.875rem] text-[var(--ms-text-body)]">
            {maskApiKey(stored)}
          </span>
          <button onClick={handleClear} className="text-[0.8rem] text-[var(--ms-text-faint)] bg-transparent border-none cursor-pointer">
            Clear
          </button>
        </div>
      )}
      ...
      <div className="flex gap-[8px]">
        ...
        />
        <button onClick={handleSave} className="py-[8px] px-[20px] rounded-[6px] bg-[var(--ms-blue)] text-white font-semibold text-[0.875rem] border-none cursor-pointer">
          {saved ? 'Saved!' : 'Save'}
        </button>
      </div>
    </main>
