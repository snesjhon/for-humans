...
export default function CheckWork({ slug }: Props) {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem(`frontend-check-result:${slug}`);
    if (stored) setResult(JSON.parse(stored));
  }, [slug]);

  const fetchHash = useCallback(async () => {
    if (!projectPath) return;
    const res = await fetch('/frontend/api/check-work/hash', {
      method: 'POST',
  ...
  async function handleCheck() {
    ...
    setResult(data);
    localStorage.setItem(`frontend-check-result:${slug}`, JSON.stringify(data));
    setChecking(false);
    await fetchHash();
  }
        ...
        <div className="space-y-3 rounded-lg border border-[var(--ms-surface)] bg-[var(--ms-bg-pane-secondary)] p-4 text-sm">
          ...
          {result.followUp && (
            <div className="border-t border-[var(--ms-surface)] pt-2">
              ...
              <p className="leading-relaxed text-[var(--ms-text-muted)]">{result.followUp}</p>
            </div>
          )}
        </div>
      )}
