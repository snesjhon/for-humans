...
export default function ScenarioPage({ params }: Props) {
  ...
  return (
    <>
      <PageHero>
        <p className="text-xs font-mono mb-2 text-[var(--ms-text-faint)]">{section.label}</p>
        <h1 className="text-5xl font-bold leading-tight text-[var(--ms-text-body)]">{scenario.label}</h1>
      </PageHero>
      ...
      <PageLayout accentColor={color} aside={<><TableOfContents headings={headings} title="Contents" /><CheckWork slug={params.slug} /></>}>
          <section className="space-y-8">
            ...
            <div className="flex items-center justify-between pt-6 border-t border-t-[var(--ms-surface)]">
              {prevScenario ? (
                <Link href={`/fullstack/scenarios/${prevScenario.slug}`} className="flex items-center gap-2 text-sm hover:opacity-70 text-[var(--ms-text-subtle)]">
                  ← {prevScenario.label}
                </Link>
              ) : (
                <Link href="/fullstack/path" className="flex items-center gap-2 text-sm hover:opacity-70 text-[var(--ms-text-subtle)]">
                  ← The Path
                </Link>
              )}
              {nextScenario ? (
                <Link href={`/fullstack/scenarios/${nextScenario.slug}`} className="flex items-center gap-2 text-sm hover:opacity-70 text-[var(--ms-text-subtle)]">
                  {nextScenario.label} →
                </Link>
              ) : (
                <div />
