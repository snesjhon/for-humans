...
function sumAddressesInRange(
  ...
  if (root.value < low) return sumAddressesInRange(root.right, low, high);
  if (root.value > high) return sumAddressesInRange(root.left, low, high);

  return (
    root.value +
    sumAddressesInRange(root.left, low, high) +
    sumAddressesInRange(root.right, low, high)
  );
}
